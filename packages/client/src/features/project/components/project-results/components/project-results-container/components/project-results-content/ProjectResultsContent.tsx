...
const ProjectResultsContent = ({ project }: ProjectResultsContentProps) => {
   ...
   return (
      <div className="grid grid-cols-2 grid-rows-2 project__results-content" aria-label={PROJECT_RESULTS_CONTENT_ARIA_LABEL}>
         <p className={`mt-12 text-2xl col-start-1 row-start-2 text-justify text-${mode === "light" ? "[#ABC4FF]" : "[#EDF2FB]"} project__results`}>{project.project_final_results}</p>
         <div className={`flex gap-20 text-2xl col-start-2 col-end-3 row-start-2 place-self-center text-justify project__results`}>
            ...
            <div className="flex flex-col">
               <span className={`mb-6 text-${mode === "light" ? "[#ABC4FF]" : "[#EDF2FB]"}`}>Number of clients</span>
               <span className={`text-2xl font-semibold text-${mode === "light" ? "[#ABC4FF]" : "[#EDF2FB]"}`}>245</span>
            </div>
         </div>
         <div className="flex flex-col col-start-2 row-start-1 project_results-inner-container">
            <span className={`text-8xl -mb-6 -z-2 font-bold ${lighestColors}`}>FINAL RESULTS</span>
            <span className={`text-8xl -mb-6 -z-1 font-bold ${lightColors}`}>FINAL RESULTS</span>
