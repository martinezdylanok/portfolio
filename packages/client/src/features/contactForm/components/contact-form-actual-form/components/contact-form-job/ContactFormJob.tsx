...
import { CONTACT_FORM_JOB_FORM_ARIA_LABEL, CONTACT_FORM_JOB_FORM_H3_TEXT, CONTACT_FORM_JOB_FORM_NAME_PLACEHOLDER, CONTACT_FORM_JOB_FORM_PHONE_PLACEHOLDER, CONTACT_FORM_JOB_FORM_EMAIL_PLACEHOLDER, CONTACT_FORM_JOB_FORM_MESSAGE_PLACEHOLDER, CONTACT_FORM_JOB_FORM_SEND_BUTTON_TEXT } from "./data/contactFormJobData";
...
const ContactFormJob = ({ activeForm }: ContactFormJobFormProps) => {
   ...
   return (
      <div className={`contact-form__actual-form-${activeForm === "secondForm" ? "active" : "inactive"}`} aria-disabled={activeForm === "secondForm" ? "false" : "true"} aria-label={CONTACT_FORM_JOB_FORM_ARIA_LABEL}>
         <h3 className={`pl-10 pt-10 text-2xl font-bold whitespace-pre-line text-${mode === "light" ? "[#ABC4FF]" : "[#EDF2FB]"}`}>{CONTACT_FORM_JOB_FORM_H3_TEXT}</h3>
               ...
               <div className={`relative ml-10 mr-10 contact-form__actual-form-input-field`}>
                  ...
                  {errors.name && (
                     <>
                        ...
                        <img src="/public/error_icon.svg" alt="Name error icon" className="absolute top-0 left-full"></img>
                     </>
                  )}
                  {!errors.name && touchedFields.name && (
                     <>
                        <img src={mode === "light" ? "/public/success_icon_dark_mode.svg" : "/public/success_icon_light_mode.svg"} className="absolute top-0 left-full" alt="Name success icon" />
                     </>
                  )}
               </div>
               <div className={`relative ml-10 mr-10 contact-form__actual-form-input-field`}>
                  ...
                  {errors.phone && (
                     <>
                        ...
                        <img src="/public/error_icon.svg" alt="Phone error icon" className="absolute top-0 left-full"></img>
                     </>
                  )}
                  {!errors.phone && touchedFields.phone && (
                     <>
                        <img src={mode === "light" ? "/public/success_icon_dark_mode.svg" : "/public/success_icon_light_mode.svg"} className="absolute top-0 left-full" alt="Phone success icon" />{" "}
                     </>
                  )}
               </div>
               <div className={`relative ml-10 mr-10 contact-form__actual-form-input-field`}>
                  ...
                  {errors.email && (
                     <>
                        ...
                        <img src="/public/error_icon.svg" alt="Email error icon" className="absolute top-0 left-full"></img>
                     </>
                  )}
                  {!errors.email && touchedFields.email && (
                     <>
                        <img src={mode === "light" ? "/public/success_icon_dark_mode.svg" : "/public/success_icon_light_mode.svg"} className="absolute top-0 left-full" alt="Email success icon" />{" "}
                     </>
                  )}{" "}
               </div>
               <div className={`ml-10 mr-10 contact-form__actual-form-input-field`}>
                  <label htmlFor="message"></label>
                  <textarea className={`text-2xl w-full transition-all duration-500 resize-none max-h-9 font-bold ${mode === "light" ? "light" : "dark"}`} placeholder={CONTACT_FORM_JOB_FORM_MESSAGE_PLACEHOLDER} id="message" name="user_message" />
               </div>
            </div>
