...
import { DUMMY_LOGO_IMAGE_URL, DUMMY_LOGO_IMAGE_URL_1, PROJECTS_LIST_CONTAINER_ARIA_LABEL_TEXT, PROJECT_LOGO_ALT_TEXT, PROJECT_SHOWCASE_IMAGE_ALT_TEXT, Project } from "./data/projectListData";
         ...
         <div className="projects-list__container" aria-label={PROJECTS_LIST_CONTAINER_ARIA_LABEL_TEXT}>
            <ul className={`p-5 flex flex-col gap-10 projects-list__list`}>
               {projects.map((project) => (
                  ...
                  <a key={project.project_id} className="projects-list__link" href={`#${project.project_name}`}>
                     <li className={`grid grid-cols-2 p-10 min-h-[100vh] border-0 rounded-xs transition duration-500 ease-in-out ${mode === "light" ? "bg-[#D7E3FC] hover:bg-[#CCDBFD]" : "bg-[#C1D3FE] hover:bg-[#CCDBFD]"} projects-list__list-item`}>
                        <div className="flex flex-col justify-between projects-list__left-side">
                           <img src={DUMMY_LOGO_IMAGE_URL_1} className="size-[5.938rem]" alt={PROJECT_LOGO_ALT_TEXT} />
                           <div className="flex-col projects-list__text-block">
                              <p className={`text-4xl -mb-5 ${mode === "light" ? "text-[#ABC4FF]" : "text-[#EDF2FB]"}`}>{project.project_technologies || "Here we will describe the concept behind it"}</p>
                              ...
                              <p className={`text-2xl ${mode === "light" ? "text-[#ABC4FF]" : "text-[#EDF2FB]"}`}>{project.project_technologies || "Techstack like this... and this, you know"}</p>
                           </div>
                        </div>
                        <div className="flex justify-end items-center projects-list__right-side">
